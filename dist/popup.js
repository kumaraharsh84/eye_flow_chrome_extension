  ...
  var require_popup = __commonJS({
    'src/popup.js'() {
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        const fieldErrorMap = {
          'eye-break-duration-sec': document.querySelector(
            '[data-error-for="eye-break-duration-sec"]'
          ),
          ...
          'subtle-reminder-fixed': document.querySelector(
            '[data-error-for="subtle-reminder-fixed"]'
          ),
        ...
        function checkIncognitoStatus() {
          const isWebhookActive =
            ...
            currentSettings.webhookUrl.trim() !== '';
          ...
          if (btnRemoveWebhook) {
            btnRemoveWebhook.addEventListener('click', () => {
              ...
              chrome.runtime.sendMessage(
                { type: 'REMOVE_WEBHOOK', removalToken: token },
                (response) => {
                  if (runtimeCallbackFailed()) {
          ...
          if (btnExportStats) {
            btnExportStats.addEventListener('click', () => {
              chrome.storage.local.get(['stats', 'runtimeState', 'auditLogs'], (data) => {
                ...
                a.href = url;
                a.download = `eyeflow-export-${/* @__PURE__ */ new Date().toISOString().slice(0, 10)}.json`;
...
`;
                content += `Sensitivity: ${settings.sensitivity !== void 0 ? settings.sensitivity : 50}%
`;
                content += '-------------------------------------------------------------\n\n';
                if (logs.length === 0) {
                  ...
                  content += logs
                    .map((l) => `[${l.timeString}] ${l.event}: ${l.details}`)
                    .join('\n');
                }
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `eyeflow-debug-logs-${/* @__PURE__ */ new Date().toISOString().slice(0, 10)}.txt`;
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                URL.revokeObjectURL(url);
        ...
        function saveSettings(options = {}) {
          ...
          chrome.runtime.sendMessage(
            { type: 'SAVE_SETTINGS', settings: nextSettings },
            (response) => {
              if (runtimeCallbackFailed()) return;
              ...
              (response) => {
                ...
                if (response.createdRemovalPin) {
                  if (webhookTokenNotice) {
                    ...
                    webhookTokenNotice.innerHTML = `\u{1F512} Webhook activated! Your Removal PIN: <b>${response.createdRemovalPin}</b><br><span style="font-size: 10px; color: var(--text-muted);">Write this PIN down! It is shown ONCE and never stored in readable format.</span>`;
                  }
                }
        ...
        function setTimingMode(mode, options = {}) {
          ...
          if (timingModeFixedButton) {
            ...
            timingModeFixedButton.setAttribute(
              'aria-pressed',
              String(currentTimingMode === 'fixed')
            );
          }
        ...
        function showSnoozeState(until) {
          ...
          snoozeUntil.textContent = date.toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
          });

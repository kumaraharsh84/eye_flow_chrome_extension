          ...
          exerciseTimer = setInterval(() => {
            ...
            if (instruction) {
              const instrIndex = Math.floor(
                ((duration - timeLeft) / duration) * instructions.length
              );
  ...
  var require_content = __commonJS({
    'src/content.js'() {
      ...
      var EyeFlowContent = (() => {
        ...
        const BREAK_DUPLICATE_GUARD_MS = 60 * 1e3;
          ...
          if (isDoomScrollContext()) {
            ...
            if (isBreakDue) {
              ...
              chrome.runtime.sendMessage({
                ...
                details: `Break cycle time: ${Math.round(getBreakCycleMs() / 1e3)}s, target: ${Math.round(sharedDsState.nextBreakTargetMs / 1e3)}s`,
              });
              lastDoomScrollTime = now;
